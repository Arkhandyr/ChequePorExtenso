...
namespace ChequePorExtenso.ClassLibrary
{
    public class Cheque
    {
        #region Arrays
        ...
        public string GerarChequePorExtenso(Decimal valor)
        {
            Decimal max = 999999999999999999999999999m;
            if (valor > max || valor <= 0)
                return "Valor não suportado";

            List<String> classes = SepararClasses(valor);

            return DesempilharClasses(EmpilharClassesPorExtenso(valor, classes));
        }

        private Stack<String> EmpilharClassesPorExtenso(decimal valor, List<string> classes)
        {
            Stack<String> classesPorExtenso = new Stack<String>();
            int iMultip = 0;
            bool redondo = VerificarSeEhRedondo(valor);
            for (int i = classes.Count; i > 0; i--)
            {
                String separador = "";
                String classeAtual = classes[i - 1];
                String multiplicador = VerificarClasseAtual(iMultip, classeAtual);
                ...
                if (classeAtual != "000" || (iMultip == 1 && valor > 1))               //permite colocar "reais" em todas os cheques que possuem valor maior ou igual 1 real
                {
                    ...
                    classesPorExtenso.Push(" " + multiplicador + separador);
                }
                iMultip++;
                classesPorExtenso.Push(GerarClassesPorExtenso(classeAtual));
            }
            return classesPorExtenso;
        }

        private string VerificarClasseAtual(int iMultip, string classeAtual)
        {
            if (classeAtual == "001") return multiplicadorSingular[iMultip];
            else return multiplicadorPlural[iMultip];
        }

        public bool VerificarSeEhRedondo(decimal valor)
        {
        ...
        private string DesempilharClasses(Stack<String> classesPorExtenso)
        {
        ...
        private string GerarClassesPorExtenso(String classe)
        {
        ...
        private List<String> SepararClasses(Decimal valor)
        {
